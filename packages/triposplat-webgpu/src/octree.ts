...
/** Must return a finite uniform variate in [0, 1). */
export type OctreeRandomSource = () => number

export interface OctreeFrontier {
  /** xyz-interleaved integer voxel coordinates at the current level. */
...
export interface OctreeOccupancyInvocation<Condition> {
  /** `[1, parentCount, 3]` xyz-interleaved normalized parent centers. */
  parentCenters: Float32Array
  parentCount: number
  level: number
  /** Child grid resolution, `2 ** level`, matching the exported `l` input. */
  resolution: number
...
export interface OctreeOccupancyOutput {
  /** `[1, parentCount, 8]` parent-major child logits. */
  logits: Float32Array
}
...
export interface OctreeSampleResult {
  ...
  points: Float32Array
  /** Leaf path log probability repeated for every emitted point. */
  logProbabilities: Float32Array
...
function randomUnit(rng: OctreeRandomSource): number {
  const value = rng()
  if (!Number.isFinite(value) || value < 0 || value >= 1) {
    throw new RangeError(`Octree RNG must return a finite value in [0, 1), got ${value}.`)
  }
  return value
}
...
function assertFrontier(frontier: Readonly<OctreeFrontier>): number {
  ...
  if (frontier.coordinates.length !== nodeCount * 3) {
    throw new RangeError(
      `Octree frontier has ${frontier.coordinates.length} coordinates for ${nodeCount} nodes.`,
    )
  }
  if (frontier.logProbabilities.length !== nodeCount) {
    throw new RangeError(
      `Octree frontier has ${frontier.logProbabilities.length} log probabilities for ${nodeCount} nodes.`,
    )
  }
  return nodeCount
}
...
/** Official row-wise systematic resampling used by the dynamic octree. */
export function systematicResample(
  probabilities: ArrayLike<number>,
  count: number,
  rng: OctreeRandomSource = Math.random,
): Uint32Array {
  if (!Number.isInteger(count) || count < 0) {
    throw new RangeError(`count must be a non-negative integer, got ${count}.`)
  }
  if (probabilities.length === 0) throw new RangeError('probabilities must not be empty.')
  // Official sample_probs performs these operations in float32. The rounding
  // points affect torch.searchsorted at exact CDF boundaries.
  const normalized = new Float32Array(probabilities.length)
  let sum = 0
  for (let index = 0; index < probabilities.length; index += 1) {
    const value = probabilities[index]
    if (!Number.isFinite(value)) {
      throw new RangeError(`Probability at index ${index} is not finite: ${value}.`)
    }
    const clamped = Math.max(0, value)
    normalized[index] = clamped
    sum = Math.fround(sum + normalized[index])
  }
  if (sum === 0) normalized.fill(Math.fround(1 / probabilities.length))
  else {
    // Official divides by row_sums.clamp_min_(1): mass above one is scaled
    // down, while a positive underfull row deliberately remains underfull.
...
/** Stable row-wise softmax and log-softmax over eight occupancy logits. */
export function octreeSoftmax(
  ...
  if (!Number.isFinite(temperature) || temperature <= 0) {
    throw new RangeError(`temperature must be positive and finite, got ${temperature}.`)
  }
  ...
  for (let parent = 0; parent < parentCount; parent += 1) {
    ...
    for (let child = 0; child < OCTREE_CHILDREN; child += 1) {
      ...
      if (!Number.isFinite(value)) throw new RangeError(`Occupancy logit ${offset + child} is not finite.`)
    ...
    for (let child = 0; child < OCTREE_CHILDREN; child += 1) {
      const value = logits[offset + child] / temperature - logDenominator
      logProbabilities[offset + child] = value
      probabilities[offset + child] = Math.exp(value)
    }
  }
  return { probabilities, logProbabilities }
}

/** Expand integer children and compact children whose sampled count is zero. */
export function expandOctreeFrontier(
  ...
  if (
    ...
    throw new RangeError(`Octree child distribution must contain ${scoreCount} values.`)
  }
  const coordinates: number[] = []
  const counts: number[] = []
  const logProbabilities: number[] = []
  // Upstream groups rows by `counts.unique()`, which is ascending on the
  // supported PyTorch backends, then draws one systematic offset for every row
  // in each group. Preserve that assignment order before compacting children;
  // drawing directly in parent order changes deterministic samples whenever
  // adjacent parents represent different final-point counts.
...
/** Correctness-first host implementation of all eight dynamic octree levels. */
export async function sampleOctree<Condition>(
  ...
  if (!Number.isInteger(levels) || levels < 1 || levels > TRIPOSPLAT_OCTREE_LEVELS) {
    throw new RangeError(`levels must be in [1, ${TRIPOSPLAT_OCTREE_LEVELS}], got ${levels}.`)
  }
  ...
  for (let level = 1; level <= levels; level += 1) {
    ...
    throwIfAborted(options.signal)
    frontier = expandOctreeFrontier(frontier, octreeSoftmax(output.logits, parentCount, temperature), rng)
    options.onLevel?.({
      level,
      totalLevels: levels,
      resolution,
  ...
  if (expanded.logProbabilities.length !== options.numPoints) {
    throw new Error(`Octree produced ${expanded.logProbabilities.length} points; expected ${options.numPoints}.`)
  }
  return { ...expanded, leaves: frontier, resolution }
}
